...
export const MobileView = ({
  ...
  <main className={styles.mainSection}>
    <div className={styles.mobileHeader}>
      Welcome {teacherData?.firstName?.data || "Teacher"}
    </div>
    <MobileUpcomingCard
      upcomingClass={upcomingClass}
      language={languageUC}
...
const LessonOverview = ({ sessionCounts }) => (
  <div className={styles.lessonOverview}>
    <div>Upcoming: {sessionCounts.upcoming}</div>
    <div>Not Scheduled: {sessionCounts.notscheduled}</div>
    <div>Cancelled: {sessionCounts.cancelled}</div>
  </div>
);
...
const RightTeacherCard = ({ teacherData, upcomingClass, language, price }) => (
  <div className={styles.rightCard}>
    {upcomingClass ? (
      <>
        <h3>{language}</h3>
        <p>{moment(upcomingClass.from).format("DD MMM YYYY")}</p>
        <p>{moment(upcomingClass.from).format("hh:mm A")}</p>
        <p>${price}</p>
      </>
    ) : (
