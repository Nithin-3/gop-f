...
function MyCalendar() {
  ...
  const getTeacherAvail = useCallback(async () => {
    ...
    if (result?.success) {
      const slots = result.data.map((el) => ({
  ...
  const addTeacherAvail = async (data) => {
    const result = await dispatch(addTeacherAvailability(data));
    if (result?.success) {
  ...
  useEffect(() => {
    ...
    getTeacherAvail();
  }, [getTeacherLoggedData, getTeacherAvail]);
    ...
    <>
      ...
      <div className="teacherCalendar">
        <Calendar
          ...
          components={{
            event: Event,
            week: { toolbar: width >= 992 ? CustomToolbar : CustomMobileToolbar },
          }}
          eventPropGetter={(event) => ({
            style: {
              backgroundColor: event.isBooked ? "gray" : "#3174ad",
