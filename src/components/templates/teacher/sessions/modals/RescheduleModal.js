...
const RescheduleModal = ({ setRescheduleModal, width }) => {
  ...
  const [setRescheduleSlot] = React.useState();

  return (
    <div className={styles.modalBackdrop}>
      <div className={styles.modal}>
        <i className={styles.closeBtn + " fas fa-close"} onClick={() => { setRescheduleModal(false); setPage("1"); }}></i>
        <h3 className={styles.modalHeading}>Request to Reschedule</h3>
        {width >= 992 ? (
          <>
            {page === "1" && <Page1 setPage={setPage} />}
            {page === "1A" && <Page1A setRescheduleModal={setRescheduleModal} setRescheduleSlot={setRescheduleSlot} setPage={setPage} />}
...
const Page1 = ({ setPage }) => (
  <form>
    <div style={{ marginTop: "20px" }}>
      ...
      <div style={{ paddingLeft: "1.5em", marginTop: "10px" }}><br />Let student select new time</div>
    </div>
    <div style={{ marginTop: "20px" }}>
      <div style={{ fontSize: "20px" }}>Message</div>
      <textarea rows="6" placeholder="Why are you requesting to reschedule the lesson? Please explain in a message to your student here." required />
    </div>
    <div style={{ marginTop: "20px", textAlign: "right" }}>
      <button type="submit" style={{ cursor: "pointer", color: "#fffefe", borderRadius: "5px", backgroundColor: "#fd869e", padding: "5px 30px", border: "none" }}>Submit</button>
    </div>
  </form>
);
  ...
  <>
    <div style={{ marginTop: "20px", border: "1px solid #d6d6d7", padding: "10px", backgroundColor: "#f1f1f1", borderRadius: "5px" }}>
      ...
      <div style={{ marginTop: "10px", display: "flex", justifyContent: "space-around", alignItems: "center" }}>
        ...
        <i className="far fa-arrow-alt-circle-right" style={{ color: "#00b63e", fontSize: "50px" }}></i>
        <div>
    ...
    <div style={{ margin: "0 auto", marginTop: "20px", display: "flex", justifyContent: "center", alignItems: "center", width: "16vw", height: "150px" }}>
      <div>
        {["", "Morning", "Afternoon", "Evening", "Night"].map((item1, index1) => (
          <div key={index1} style={{ display: "flex", justifyContent: "space-between" }}>
            <div style={{ marginRight: "10px" }}>{item1}</div>
            <div style={{ display: "flex", justifyContent: "center", gap: "3px" }}>
              {["Su", "Mo", "Tu", "Th", "We", "Fr", "Sa"].map((item2, index2) =>
                index1 === 0 ? (
                  <div key={index2} style={{ marginBottom: "5px", width: "1.5vw", height: "1.5vw", textAlign: "center" }}>{item2}</div>
                ) : (
                  <div key={index2} style={{ width: "1.5vw", height: "1.5vw", backgroundColor: index1 === 2 ? "#359cd7" : index1 === 4 ? "#e7f1f9" : "#9fcce6" }}></div>
                )
              )}
            </div>
          </div>
