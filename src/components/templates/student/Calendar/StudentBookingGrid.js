      ...
      <div className={modalStyles.modal} onClick={e => e.stopPropagation()} style={{ overflow: "hidden" }}>
        ...
        <div style={{ padding: "30px", textAlign: "center" }}>
          ...
          <div style={{ backgroundColor: "#f9f9f9", borderRadius: 12, padding: 20, marginBottom: 30 }}>
            <p style={{ fontWeight: "600", color: "#444", margin: "5px 0" }}>{format(event.start, "EEEE, MMMM do, yyyy")}</p>
            <p style={{ fontWeight: "600", color: "#fe1848", fontSize: "1.1rem" }}>{format(event.start, "hh:mm a")} - {format(event.end, "hh:mm a")}</p>
          </div>
          <button style={{ width: "100%", borderRadius: "30px", padding: "15px", background: "#dc3545", color: "#fff", border: "none", fontWeight: "bold", fontSize: "1.1rem" }} onClick={() => confirmBooking(event)}>Confirm & Proceed</button>
        </div>
      </div>
    </div>
  );
